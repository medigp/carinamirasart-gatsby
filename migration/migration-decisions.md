...
- about and press already exist in Directus and remain append-only SKIP.
- exhibitions remains explicitly excluded from the legacy migration even though a Directus page exists.
- Page-level Markdown/MDX body is migrated only from the document body; paragraphs[] is never folded into it.
- about.paragraphs[] is reserved for the later biography migration.
- press.paragraphs[] is reserved for the later press migration.
- reviews is completely EXCLUDED, including reviews.paragraphs[], reviews.jpg and all legacy SEO. Do not create pages.reference=reviews or any placeholder. A future reviews feature requires new development and, if needed, a dedicated Directus model.
- The current pages.main_image and pages.seo_image schema fields do not configure a folder UUID. The importer must not invent one; a null schema folder means an upload without an explicit folder.
### Final pages coverage

| Legacy field or content | Status | Decision |
|---|---|---|
| reference | IMPLEMENTED | pages.reference with append-only lookup |
| hide/status | IMPLEMENTED | true → archived; false/absent → published |
| title, subtitle, body | IMPLEMENTED | Catalan pages_translations fields |
| SEO text and keywords | IMPLEMENTED | Catalan pages_translations SEO fields |
| seo.image | IMPLEMENTED | pages.seo_image with SHA-256 file-map |
| pageName, url | DERIVED | generated by Gatsby |
| lastModificationDate | EXCLUDED | Directus owns audit timestamps |
| about.paragraphs[] | EXCLUDED FROM PAGES | reserved for biography_events |
| press.paragraphs[] | EXCLUDED FROM PAGES | reserved for press_articles |
| reviews and reviews.paragraphs[] | EXCLUDED | no page, file, SEO or placeholder |
| exhibitions legacy | EXCLUDED | no legacy import |
